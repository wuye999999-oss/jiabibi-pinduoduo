...
function postForm(url, params) {
  return new Promise((resolve, reject) => {
    ...
    const req = https.request({
      ...
      res.on('end', () => {
        try { resolve(JSON.parse(data)); } catch (err) { reject(new Error('API returned non-JSON: ' + data.slice(0, 300))); }
      });
    });
    req.on('error', reject);
    req.write(body);
...
async function pddRequest(type, bizParams = {}) {
  if (!CLIENT_ID || !CLIENT_SECRET || !PDD_PID) return { error: 'missing_env', message: 'Missing PDD_CLIENT_ID, PDD_CLIENT_SECRET, or PDD_PID in Render Environment Variables.' };
  const params = cleanParams({ type, client_id: CLIENT_ID, timestamp: Math.floor(Date.now() / 1000).toString(), data_type: 'JSON', ...bizParams });
...
async function jdRequest(method, bizObject = {}) {
  if (!JD_APP_KEY || !JD_APP_SECRET) return { error: 'missing_jd_env', message: 'Missing JD_APP_KEY/JD_APPKEY or JD_APP_SECRET/JD_SECRET in Render Environment Variables.' };
...
function fenToYuan(value) { return Math.round(Number(value || 0)) / 100; }
...
function normalizeGoods(item) {
  ...
  return {
    platform: 'pdd', goods_name: item.goods_name, goods_desc: item.goods_desc, brand_name: item.brand_name || '',
    ...
    min_group_price: minGroupPrice, min_group_price_yuan: fenToYuan(minGroupPrice),
    coupon_discount: couponDiscount, coupon_discount_yuan: fenToYuan(couponDiscount),
    coupon_price: couponPrice, coupon_price_yuan: fenToYuan(couponPrice),
    has_coupon: Boolean(item.has_coupon), unified_tags: item.unified_tags || [], promotion_rate: item.promotion_rate || 0, raw: item,
  };
}
...
function normalizeJdGoods(item) {
  ...
  return {
    platform: 'jd', goods_name: item.skuName || item.goodsName || item.title || '', goods_desc: item.skuName || item.goodsName || item.title || '',
    ...
    coupon_url: coupon?.link || coupon?.couponUrl || '', sales_tip: item.inOrderCount30Days ? String(item.inOrderCount30Days) : (item.comments || ''),
    min_group_price_yuan: price, coupon_discount_yuan: couponDiscount, coupon_price_yuan: finalPrice,
    has_coupon: Boolean(coupon), unified_tags: [coupon ? '有优惠券' : '', item.owner === 'g' ? '京东自营' : '', item.shopInfo?.shopName || ''].filter(Boolean),
    promotion_rate: item.commissionInfo?.commissionShare || 0, raw: item,
  };
}

function parseJdQueryResult(result, responseKey) {
  const wrapper = result[responseKey] || result.jd_union_open_goods_query_responce || result.jd_union_open_goods_query_response || result.jd_union_open_goods_jingfen_query_responce || result.jd_union_open_goods_jingfen_query_response || result;
...
function isJdForbidden(queryResult) { return String(queryResult?.code || '') === '403' || /无访问权限/.test(String(queryResult?.message || '')); }
...
const server = http.createServer(async (req, res) => {
  try {
    ...
    if (url.pathname === '/' || url.pathname === '/health') return sendJson(res, 200, {
      ok: true, name: '价比比 API', message: '价比比 API 已启动',
      env: { has_client_id: Boolean(CLIENT_ID), has_client_secret: Boolean(CLIENT_SECRET), has_pid: Boolean(PDD_PID), has_jd_app_key: Boolean(JD_APP_KEY), has_jd_app_secret: Boolean(JD_APP_SECRET), has_jd_position_id: Boolean(JD_POSITION_ID), jd_api_url: JD_API_URL },
    });
    ...
    if (url.pathname === '/api/pdd/search' && req.method === 'GET') {
      ...
      const result = await pddRequest('pdd.ddk.goods.search', { keyword, page, page_size: pageSize, with_coupon: 'true', pid: PDD_PID, custom_parameters: CUSTOM_PARAMETERS, sort_type: sortType });
    ...
    if (url.pathname === '/api/pdd/link' && req.method === 'POST') {
      ...
      const result = await pddRequest('pdd.ddk.goods.promotion.url.generate', { p_id: PDD_PID, goods_sign_list: JSON.stringify([goodsSign]), generate_short_url: 'true', generate_mobile: 'true', generate_schema_url: 'true', generate_we_app: 'true', custom_parameters: CUSTOM_PARAMETERS });
      if (result.error_response || result.error) return sendJson(res, 400, result);
    ...
    if (url.pathname === '/api/jd/search' && req.method === 'GET') {
      ...
      const goodsQueryRaw = await jdRequest('jd.union.open.goods.query', { goodsReqDTO: { keyword, pageIndex: page, pageSize: Math.min(pageSize, 30), sceneId: 1, isCoupon: 1, hasBestCoupon: 1, pid: JD_PID } });
      let queryResult = parseJdQueryResult(goodsQueryRaw, 'jd_union_open_goods_query_responce');
      let source = 'goods.query';
      let raw = goodsQueryRaw;

      if (isJdForbidden(queryResult)) {
        const eliteId = Number(url.searchParams.get('elite_id') || '1');
        const jingfenRaw = await jdRequest('jd.union.open.goods.jingfen.query', { goodsReq: { eliteId, pageIndex: page, pageSize: Math.min(pageSize, 30), pid: JD_PID } });
        queryResult = parseJdQueryResult(jingfenRaw, 'jd_union_open_goods_jingfen_query_responce');
        source = 'jingfen.query.fallback';
        raw = { goods_query_error: queryResult, jingfen: jingfenRaw };
      }

      if (queryResult?.code && String(queryResult.code) !== '200') return sendJson(res, 400, { ok: false, platform: 'jd', keyword, jd_error: queryResult, source, raw });
      ...
      return sendJson(res, 200, { ok: true, platform: 'jd', keyword, source, total_count: totalCount, goods_list: list, raw });
    }
