...
function jdTimestamp() { const now = new Date(); const utc = now.getTime() + now.getTimezoneOffset() * 60000; const bj = new Date(utc + 8 * 3600000); const p = n => String(n).padStart(2, '0'); return `${bj.getFullYear()}-${p(bj.getMonth() + 1)}-${p(bj.getDate())} ${p(bj.getHours())}:${p(bj.getMinutes())}:${p(bj.getSeconds())}`; }
async function pddRequest(type, biz = {}) { if (!PDD_CLIENT_ID || !PDD_CLIENT_SECRET || !PDD_PID) return { error: 'missing_pdd_env' }; const params = cleanParams({ type, client_id: PDD_CLIENT_ID, timestamp: Math.floor(Date.now() / 1000), data_type: 'JSON', ...biz }); params.sign = makeSign(params, PDD_CLIENT_SECRET.trim()); return postForm(PDD_API_URL, params); }
async function jdRequest(method, biz = {}) { if (!JD_APP_KEY || !JD_APP_SECRET) return { error: 'missing_jd_env' }; const params = cleanParams({ method, app_key: JD_APP_KEY, access_token: JD_ACCESS_TOKEN, timestamp: jdTimestamp(), format: 'json', v: '1.0', sign_method: 'md5', '360buy_param_json': JSON.stringify(biz) }); params.sign = makeSign(params, JD_APP_SECRET); return postForm(JD_API_URL, params); }
...
function cleanShareKeyword(input) {
  let text = decodeText(input)
    ...
    .replace(/\s+/g, ' ')
    .trim();
  const chineseRuns = text.match(/[\u4e00-\u9fa5A-Za-z0-9\.\-\+（）()]+/g) || [];
  text = chineseRuns.join(' ').replace(/\s+/g, ' ').trim();
  const stop = ['抖音商城', '淘宝', '天猫', '京东', '拼多多', '复制', '搜索'];
  for (const s of stop) text = text.replace(new RegExp(s, 'g'), ' ');
...
function parseProductLink(input) {
  ...
  const out = { is_link: /^https?:\/\//i.test(url), platform: '', id: '', id_type: '', url, keyword_hint: cleanShareKeyword(text) };
  if (!out.is_link) return out;
  let u = null; try { u = new URL(url); } catch { return out; }
  ...
  if (/jd\.com|3\.cn/.test(host)) {
    ...
    out.platform = 'pdd'; out.id = u.searchParams.get('goods_id') || u.searchParams.get('goodsId') || u.searchParams.get('goods_sign') || u.searchParams.get('goodsSign') || ''; out.id_type = /[a-zA-Z_]/.test(out.id) ? 'goods_sign' : 'goods_id';
...
async function searchPddItemByLink(parsed) { if (!parsed.id) return providerPlaceholder('pdd', parsed.url, '拼多多链接已识别，但未提取到 goods_id/goods_sign。'); const biz = { pid: PDD_PID }; if (parsed.id_type === 'goods_sign') biz.goods_sign = parsed.id; else biz.goods_id_list = JSON.stringify([Number(parsed.id)]); const raw = await pddRequest('pdd.ddk.goods.detail', biz); if (raw.error_response || raw.error) throw raw; const response = raw.goods_detail_response || raw; const list = (response.goods_details || response.goods_list || []).map(item => normalizePdd(item, 'url.detail')); return { ok: true, platform: 'pdd', source: 'url.detail', keyword: parsed.id, total_count: list.length, goods_list: list, raw }; }
async function resolveLink(input) { const parsed = parseProductLink(input); if (!parsed.is_link) return { ok: true, is_link: false, keyword: cleanShareKeyword(input) || String(input || '').trim(), goods_list: [], parsed }; let exact = { ok: true, platform: parsed.platform, source: 'link_detected', goods_list: [], total_count: 0 }; try { if (parsed.platform === 'jd' && parsed.id) exact = await searchJdItemBySku(parsed.id); else if (parsed.platform === 'pdd' && parsed.id) exact = await searchPddItemByLink(parsed); else if (parsed.platform === 'tb') exact = providerPlaceholder('tb', parsed.keyword_hint || parsed.id || input, '淘宝链接已识别，真实商品详情 provider 待接入。'); else if (parsed.platform === 'douyin') exact = providerPlaceholder('douyin', parsed.keyword_hint || parsed.id || input, '抖音链接已识别，真实商品详情 provider 待接入。'); } catch (err) { exact = { ok: false, platform: parsed.platform, source: 'link_resolve_failed', error: err.error || err.message || 'resolve_failed', detail: err, goods_list: [] }; } const first = exact.goods_list?.[0]; const seedKeyword = first?.goods_name || parsed.keyword_hint || cleanShareKeyword(input) || parsed.id || String(input || '').trim(); return { ok: true, is_link: true, parsed, seed_keyword: seedKeyword, exact }; }
...
async function searchJd(keyword, page = 1, pageSize = 20) { const sku = extractJdSku(keyword); if (sku) return searchJdItemBySku(sku); const goodsRaw = await jdRequest('jd.union.open.goods.query', { goodsReqDTO: { keyword, pageIndex: page, pageSize: Math.min(pageSize, 30), sceneId: 1, isCoupon: 1, hasBestCoupon: 1, pid: JD_PID } }); let qr = parseJdQueryResult(goodsRaw, 'jd_union_open_goods_query_responce'); let source = 'goods.query'; let raw = goodsRaw; if (isForbidden(qr)) { const jfRaw = await jdRequest('jd.union.open.goods.jingfen.query', { goodsReq: { eliteId: 1, pageIndex: page, pageSize: Math.min(pageSize, 30), pid: JD_PID } }); qr = parseJdQueryResult(jfRaw, 'jd_union_open_goods_jingfen_query_responce'); source = 'jingfen.query.fallback'; raw = { goods_query_error: parseJdQueryResult(goodsRaw, 'jd_union_open_goods_query_responce'), jingfen: jfRaw }; } if (qr?.code && String(qr.code) !== '200') throw { jd_error: qr, raw }; const list = extractJdGoodsList(qr).map(item => normalizeJd(item, source)); return { ok: true, platform: 'jd', source, keyword, total_count: Number(qr?.totalCount || list.length), goods_list: list, raw }; }
...
function inferBrand(keyword) { const brands = ['小米', '安克', '维达', '蓝月亮', '苹果', '华为', '荣耀', '美的', '得力', '公牛', '飞利浦', '可口可乐', '雪碧', '芬达']; return brands.find(b => keyword.includes(b)) || ''; }
function inferCategory(keyword) { const cats = ['充电宝', '纸巾', '洗衣液', '耳机', '手机壳', '电饭煲', '文具', '保温杯', '汽水', '饮料', '碳酸饮料']; return cats.find(c => keyword.includes(c)) || keyword || '商品'; }
function mockGoods(platform, keyword, index, price, tag, shop) { const brand = inferBrand(keyword); const cat = inferCategory(keyword); return { platform, source: 'mock', goods_name: `${brand ? brand + ' ' : ''}${cat} ${tag} Mock ${index}`, goods_desc: '开发调试 Mock 商品，不用于正式展示', brand_name: brand || platform, shop_name: shop, goods_image_url: `https://placehold.co/600x600?text=${encodeURIComponent(platform)}`, goods_thumbnail_url: `https://placehold.co/300x300?text=${encodeURIComponent(platform)}`, goods_id: `${platform}_mock_${index}`, sales_tip: `${index * 1000}+`, min_group_price_yuan: price + 20, coupon_discount_yuan: 20, coupon_price_yuan: price, has_coupon: true, unified_tags: ['Mock', tag, shop], material_url: '', raw: { mock: true } }; }
...
async function unifiedSearch(inputKeyword, platforms, page, pageSize, useMock = false) { const linkResolved = useMock ? null : await resolveLink(inputKeyword); const keyword = linkResolved?.is_link ? linkResolved.seed_keyword : (cleanShareKeyword(inputKeyword) || inputKeyword); const wanted = platforms.includes('all') ? ['pdd', 'jd', 'tb', 'douyin'] : platforms; const tasks = wanted.map(async platform => { const p = platform === 'taobao' ? 'tb' : platform === 'dy' ? 'douyin' : platform; try { if (useMock) return searchMock(p, keyword); if (p === 'pdd') return await searchPdd(keyword, page, pageSize); if (p === 'jd') return await searchJd(keyword, page, pageSize); if (p === 'tb') return await searchTaobao(keyword); if (p === 'douyin') return await searchDouyin(keyword); return { ok: false, platform: p, error: 'unknown_platform', goods_list: [] }; } catch (err) { return { ok: false, platform: p, error: err.error || err.message || 'provider_error', detail: err, goods_list: [] }; } }); const results = await Promise.all(tasks); let goods = results.flatMap(r => r.goods_list || []); if (linkResolved?.exact?.goods_list?.length) { const seen = new Set(goods.map(g => `${g.platform}:${g.goods_id || g.sku_id || g.goods_sign || g.goods_name}`)); for (const x of linkResolved.exact.goods_list.map(x => ({ ...x, source: `${x.source}.exact_input` }))) { const key = `${x.platform}:${x.goods_id || x.sku_id || x.goods_sign || x.goods_name}`; if (!seen.has(key)) goods.unshift(x); } } return { ok: true, input: inputKeyword, keyword, link: linkResolved?.is_link ? linkResolved : null, mock: Boolean(useMock), platforms: wanted, total_count: goods.length, providers: results.map(({ ok, platform, source, total_count, error, message }) => ({ ok, platform, source, total_count: total_count || 0, error, message })), goods_list: goods }; }
...
const server = http.createServer(async (req, res) => {
  try {
    ...
    if (url.pathname === '/' || url.pathname === '/health') return sendJson(res, 200, { ok: true, name: '价比比 API', message: '价比比 API 已启动', env: { has_client_id: Boolean(PDD_CLIENT_ID), has_client_secret: Boolean(PDD_CLIENT_SECRET), has_pid: Boolean(PDD_PID), has_jd_app_key: Boolean(JD_APP_KEY), has_jd_app_secret: Boolean(JD_APP_SECRET), has_jd_position_id: Boolean(JD_POSITION_ID), tb_provider: 'placeholder_no_fake_goods', douyin_provider: 'placeholder_no_fake_goods', mock_mode: 'use &mock=1 explicitly', link_resolve: ['jd', 'pdd', 'tb', 'douyin'], share_text_clean: true } });
